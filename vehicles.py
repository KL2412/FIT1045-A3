...
class CrappyCrepeCar(Vehicle):
    ...
    def __init__(self, speed: int) -> None:
        ...
        """
        #create instance variable speed
        self.speed = speed
    ...
    def compute_travel_time(self, departure: City, arrival: City) -> float:
        ...
        Return Value: 
            float value
        Function Body:
            calculate travel time and return the value
        """
...
class DiplomacyDonutDinghy(Vehicle):
    ...
    def compute_travel_time(self, departure: City, arrival: City) -> float:
        ...
        Return Value: 
            float value
        Function Body:
            check if the two cities are in same city
            check if the two cities are both primary in diff countries
            determine if the travel is possible
            if two locations are in same country:
                calculate time with in_country_speed
            elif two locations are the primary city of diff country:
                calculate time with between_primary_speed
            else:
                time is infinity
            return time
        """
...
class TeleportingTarteTrolley(Vehicle):
    ...
    def compute_travel_time(self, departure: City, arrival: City) -> float:
        ...
        Function Body:
            ...
            if possible return travel time
            if not possible return infinity
        """
        possible_travel = departure.distance(arrival) <= self.distance_limit - 0.00001
        if possible_travel:
